...
public class TrabajaFacadeREST extends AbstractFacade<Trabaja> {
    ...
    /**
     * Obtiene la clave primaria a partir de un segmento de ruta URI.
     *
     * @param pathSegment Segmento de ruta URI que contiene la matriz de parámetros.
     * @return Clave primaria de la entidad Trabaja.
     */
    private TrabajaId getPrimaryKey(PathSegment pathSegment) {
        TrabajaId key = new TrabajaId();
        MultivaluedMap<String, String> map = pathSegment.getMatrixParameters();
        List<String> id_sede = map.get("id_sede");
        if (id_sede != null && !id_sede.isEmpty()) {
            key.setId_sede(new Integer(id_sede.get(0)));
        }
        List<String> ud_User = map.get("ud_User");
        if (ud_User != null && !ud_User.isEmpty()) {
            key.setUd_User(new Integer(ud_User.get(0)));
        }
        return key;
    }
     ...
     * AbstractFacade inicializando la clase base con la entidad Trabaja.
     */
    ...
    public void remove(@PathParam("id") PathSegment id) {
        TrabajaId key = getPrimaryKey(id);
